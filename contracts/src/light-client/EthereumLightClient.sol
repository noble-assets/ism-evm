...
contract EthereumLightClient is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, IEthereumLightClient {
     ...
     * @dev Verifies an SP1 proof that attests to a valid light client state transition from a
     *      previously known header and sync committee to a new finalized header. The proof
     *      encapsulates BLS signature verification over the sync committee and Merkle proofs
     *      for the finalized header and execution payload.
     *
     *      The update can span multiple sync committee periods if the proof includes the
     *      necessary intermediate updates. Sync committees are stored by period and the
     *      next sync committee is cached when available.
     *
     * @param proof The SP1 proof bytes generated by the Helios light client circuit.
     ...
     * @dev Can only be called by the contract owner. Use in case of detected issues or attacks.
     */
     ...
     * @dev This function can only be called by the contract owner. Use when upgrading the
     *      SP1 circuit to a new version that produces proofs with a different verification key.
