...
use helios_ethereum::{
    config::{Config, checkpoints::CheckpointFallback, networks::Network},
    consensus::Inner,
...
pub async fn get_latest_checkpoint(chain_id: u64) -> Result<B256> {
    ...
    cf.fetch_latest_checkpoint(&network)
        .await
        .map_err(|e| anyhow!("error fetching latest checkpoint: {e}"))
}
...
pub async fn get_client(
    ...
    client
        .bootstrap(root)
        .await
        .map_err(|e| anyhow!("error bootstrapping client: {e}"))?;
